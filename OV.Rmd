...
authors: "Óskar Höskuldsson og Viktor Sigbjörn Víðisson"
...
```{r}
ggplot(data=eignir, aes(x=eignir$byggar, fill=eignir$svfn)) + geom_bar() + labs(x='Sveitafélög', y= "Fjöldi", fill = "Tegund eignar")
```
...
```{r}
ggplot(data=eignir, aes(x=eignir$byggar, fill=eignir$svfn)) + geom_bar()
```
...
ggplot(data=serbyli, aes(x=serbyli$birtm2, fill=serbyli$svfn)) + geom_histogram()
```
...
```{r}
ggplot(data=eignir, aes(x=fermetraverd, fill=svfn)) + geom_histogram()
```
